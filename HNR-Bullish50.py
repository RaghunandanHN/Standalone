#######################################################################
###################### NIFTY 50 Bullish BOT - Single Pass ######################
#######################################################################
...
log_file_path = os.path.join(log_directory, datetime.now().strftime('%d-%m-%Y') + " Nifty_50_Bullish.log")
...
def run_bullish50_screener():
    """Single pass execution for NIFTY 50 Bullish screener"""
    
    screenType = "Nifty 50 Bullish"
    screenContents = "NIFTY 50"
    alertCounter = 1
    listi = []
    ...
    data = {
        'scan_clause': '( {33492} ( latest close > 1 day ago close and latest rsi( 14 ) > 55 and weekly rsi( 14 ) > 55 and monthly rsi( 14 ) > 55 and weekly rsi( 14 ) > 1 week ago rsi( 14 ) and [0] 1 hour rsi( 14 ) > [0] 1 hour ema ( [0] 1 hour rsi( 14 ) , 14 ) and latest close > 100 and latest volume > 50000 ) ) '
    }
    ...
    try:
        with requests.Session() as s:
            ...
            if len(df) > 0:
                ...
                # Telegram setup
                TOKEN = '7688618254:AAEzkjBPcglzYu_x-grL81twDwoOdMC9s38'
                URL = f"https://api.telegram.org/bot{TOKEN}/"
                chat_id = "-1002365501285"
