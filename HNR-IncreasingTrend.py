#######################################################################
###################### Increasing Trend BOT - Single Pass ######################
#######################################################################
...
log_file_path = os.path.join(log_directory, datetime.now().strftime('%d-%m-%Y') + " Nifty_Increasing_Trend.log")
...
def run_increasing_trend_screener():
    """Single pass execution for Increasing Trend screener"""
    
    screenType = "Increasing Trend"
    ...
    data = {
        'scan_clause': '( {57960} (latest rsi( 14 ) > 55 and weekly rsi( 14 ) > 60 and monthly rsi( 14 ) > 60 and [0] 15 minute rsi( 14 ) > 55 and [-1] 15 minute rsi( 14 ) > 55 and [-2] 15 minute rsi( 14 ) > 55 and latest close > 100 and [0] 15 minute "close - 1 candle ago close / 1 candle ago close * 100" > 0 and latest "close - 1 candle ago close / 1 candle ago close * 100" > 0 and [0] 15 minute close > 100 and latest volume > 100000 and [0] 15 minute volume > 100000 and latest sma ( latest close , 20 ) - 4 weeks ago sma ( 4 weeks ago close , 20 ) / 100 > 0.05 and 4 weeks ago sma ( 4 weeks ago close , 20 ) - 8 weeks ago sma ( 8 weeks ago close , 20 ) / 100 > 0.05 and latest adx di positive ( 14 ) > 25 and [0] 15 minute adx di positive ( 14 ) > 25 and [-1] 15 minute adx di positive ( 14 ) > 25 and [-2] 15 minute adx di positive ( 14 ) > 25 ) ) '
    }
    ...
    try:
        with requests.Session() as s:
            ...
            if len(df) > 0:
                ...
                # Telegram setup
                TOKEN = '6657875319:AAHmu1dpcGQ022xxJ7S_KCKXz2N52X6esxE'
                URL = f"https://api.telegram.org/bot{TOKEN}/"
                chat_id = "@AlertzChannel"
